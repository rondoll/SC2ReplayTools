...
import static com.rondoll.sc2replayrenamercli.ArgumentConstant.*;
		...
		try {
			...
			for (String str : args) {
				...
				if (str.toLowerCase().startsWith(RULE)) {
					renamingRule.setRule(value);
				} else if (str.toLowerCase().startsWith(SOURCE)) {
					source = new File(value);
				} else if (str.toLowerCase().startsWith(TARGET)) {
					target = new File(value);
				} else if (str.toLowerCase().startsWith(KEEP_ORIGINAL) && value.toLowerCase().equals("false")) {
					keepOriginal = false;
				} else if (str.toLowerCase().startsWith(PLAYER)) {
					renamingRule.setPlayer(value);
				} else if (str.toLowerCase().startsWith(RACE)) {
					renamingRule.setRace(value);
				} else if (str.toLowerCase().startsWith(PLAYER_AND_RACE)) {
					renamingRule.setPlayerAndRace(value);
				} else if (str.toLowerCase().startsWith(PLAYERS_DIVIDER)) {
					renamingRule.setPlayersDivider(value);
				} else if (str.toLowerCase().startsWith(RACES_DIVIDER)) {
					renamingRule.setRacesDivider(value);
				} else if (str.toLowerCase().startsWith(PLAYERS_AND_RACES_DIVIDER)) {
					renamingRule.setPlayersAndRacesDivider(value);
				} else if (str.toLowerCase().startsWith(SHORT_RACE_NAME) && "false".equals(value.toLowerCase())) {
					//only checks for false since short race name is true by default
					renamingRule.setShortRaceName(false);
				} else if (str.toLowerCase().startsWith(PRIORITIZED_PLAYERS)) {
					renamingRule.setPrioritizedPlayers(value.split(" "));
				} else {
					System.out.println(WRONG_ARGUMENTS + "\n");
					showHelp();
					return;
